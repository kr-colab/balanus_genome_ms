...
are seen across synonymous sites, where substitutions are normally considered as neutral.
...
from the population.
Others repeats might be beneficial genetic polymorphisms, as transposable elements are
sometimes known to play a role in adaptive evolution \citep{schrader_impact_2019}.
These repeat elements can act as regulators of gene expression, e.g.,
...
$\alpha_\text{asym}$ is approximately 0.57 (\cite{messer_frequent_2013}).
...
per-gene $\alpha$ \cite{smith_adaptive_2002}, Neutrality Index (NI;
...
polymorphism, and estimating $\alpha_\text{asym}$ aggregated across
all genes.
Lastly, we calculated the Tarone-Greenland weighted estimator
($\alpha_\text{TG}$;~\cite{stoletzki_estimation_2011}) using
...
After determining degeneracy we separatelty calculated $\pi$ for 0-fold and 4-fold
