...
and more stringent purging of haplotig and contaminant sequences (see
\nameref{sec:methods_assembly}), we generated contigs with a substantially smaller
fraction of haplotigs, reducing the assembly size to 1.05 Gbp and the \textsc{BUSCO}
duplicates to 4.84\%.
This contig-level assembly was then scaffolded with Hi-C data using the \textsc{YaHS}
software (\cite{zhou_yahs_2023}; Figure~\ref{fig:sup_hic}), generating an assembly
...
\subsection*{Patterns of molecular evolution reflect positive selection in
...
Secondly, we calculated $\alpha$ using the asymptotic McDonald-Kreitman test
($\alpha_\text{asym}$), which corrects for the presence of weakly deleterious
non-synonymous polymorphisms present at low frequency in the population
\citep{messer_frequent_2013} which might obscure the signal of positive selection.
We found a mean $\alpha_\text{asym}$ of 0.616 ($95\%$ CI: 0.610--0.622), a
larger proportion to that observed in \textit{Drosophila} ($\alpha_\text{asym}$
...
We sequenced the whole genomes of six \bgla{} individuals from the central Oregon coast.
After stringent filtering of sites with low coverage, low quality, and/or missing
genotypes, we retained a total of 111.6 million genotyped sites
(Table~\ref{tab:sup_popgen_stats}), 21.4 million (19.16\%) of which were variant in the
sequenced individuals.
Among these variant sites, we recovered 20.5 million point mutations, including nearly 
1 million SNPs that were multiallelic in this set of just six individuals.
...
at zero-fold degenrate ($\pi_{0}$), in which any mutation produces a non-synonymous
substitution, and four-fold degenerate sites ($\pi_{4}$), in which any mutation results
...
and the intregation of Hi-C and ultra-long read data for phased telomere-to-telomere
...
% @Andy: Added another method and a concluding sentence stating that, despite the gaps,
% our genome is a step in the right direction. Not the most meaningful statement, but I
% wanted to end with something else.
...
We see this patterns reflected in the \bgla{} genome.
...
Table~\ref{tab:sup_feature_pi}) when compared to less constrained features of the genome.
...
Increased efficacy of purifying selection is also reflected by our the results of Our
McDonald-Kreitman test (Figure~\ref{fig:mkTest}).
We observe a relative decrease in the proportion of non-synonymous polymorphism observed
in a central Oregon \bgla{} population, suggesting an increase in the number of
weakly deleterious polymorphism purged by selection \citep{li_reverse_2008}.
Positive selection is also expected to increase in efficacy in large populations,
resulting in a the fixation of a large fraction of non-synonymous mutations.
We observe also this pattern in the \bgla{} genome as reflected by the McDonald-Kreitman
test (Figure~\ref{fig:mkTest}).
In this central Oregon population, the majority (54 to 61\%, depending on the metric
of $\alpha$ used) of non-synonymous mutations reaching fixation due to due to the
effects of positive selection, a proportion that is higher to that observed in diverse
\textit{Drosophila} populations \citep{haller_asymptoticmk_2017}.
\vspace{0.2cm}

In large enough populations, selection should be efficacious enough that is effects
are seen across synonymous and non-coding sites.
This could result in a bias in the usage of synonymous codons, as different codons
are favored over others due to GC biases, tRNA abundance, and other factors
...
On average, \bgla{} uses $\approx$ 43 out of 61 available codons, a larger reduction
to the one observed in \textit{D. melanogaster} (Figure~\ref{fig:sup_dmel_enc}).
This strong biases in codon usage further reflects increase in efficacy of natural
selection due to the large effective of Pacific acorn barnacle populations.
\vspace{0.2cm}
...
Some could be low-frequency, slightly deleterious variants not yet purged from
the population.
Others repeats might be beneficial genetic polymorphisms.
Transposable elements can have known impacts on adaptive evolution
\citep{schrader_impact_2019}.
These repeat elements can act as key regulators of gene expression, e.g.,
\cite{trizzino_transposable_2017,rech_population-scale_2022}, and as such might be
respondible for the necessary plasticity required for survival in rapidly changing
...
% Andy: Check these new examples.
Regardless of their role, disentangling the population-wide repeat landscape and its fitness
effects is likely impossible with a single haploid reference assembly.
...
% TODO: mention the proportion of reads aligned
