...
\texttt{116172}).
This process removed 236 spanning $\approx$ 19.7 Mbp, resting in an assembly with
2,696 contigs, a total size of 1.18 Gbp, and a contig N50 of 758.4 kbp.
...
downstream analysis.
This RefSeq annotation contains 20,420 protein-coding genes and is 88.72\% gene-complete,
with 35.93\% duplicates, when validated against the \texttt{arthropoda\_odb10} reference
ortholog dataset using \textsc{hifiasm} \texttt{protein}.
...
\texttt{GCA\_037642225.1}), no annotation has been published for this assembly.
Therefore, we then took the published RefSeq annotation (NCBI accession:
\texttt{GCF\_019059575.1}), based on the scaffold-level genome assembly by
\cite{kim_draft_2019}.
...
(NCBI accession: \texttt{GCA\_964274985.1}; \citep{bishop_genome_2025}) using publicly
...
\citep{kriventseva_orthodb_2019,zdobnov_orthodb_2021}.
In \textsc{BRAKER}, we enforced the recovery of orthologs from the Arthropod \textsc{BUSCO}
dataset in the annotation (\texttt{--busco\_lineage=arthropoda\_odb10}).
...
with 16.36\% after validation with \textsc{compleasm} against the \texttt{arthropoda\_odb10}
reference ortholog dataset.
...
\citep{chen_capmit_2021}) using RNAseq short read data publicly available on the NCBI
SRA database (Table~\ref{tab:sup_sra_data}).
...
However, given the comparatively smaller number of annotated sequences, and the resulting
reduction in the number of orthologous sequences recovered when incorporting this species,
we excluded \textit{C. mitella} from most downstream analyses.
