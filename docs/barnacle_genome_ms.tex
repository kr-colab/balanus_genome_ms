...
software \citep{baril_earl_2024}. This analysis identified 698.6 Mbp (66.94\%)
...
sites among our conserved sites; however, this is likely the product of the
whole-genome alignment, given rRNA's repetitive nature and alignment difficulty
\citep{gillespie_characterizing_2004}, and not an accurate measure of conservation (or
lack thereof).
...
barnacles.
In \textit{Drosophila}, conserved elements cover 44.5\% of the genome, with 34\% of conserved
bases falling in protein-coding exons \citep{siepel_evolutionarily_2005}.
...
compared with $\approx$15\% in \textit{D. melanogaster}), which contributes a large fraction
of HCE bases in transposable element and overlapping annotation categories.
...
McDonald-Kreitman test \citep{mcdonald_adaptive_1991}. Specifically, we used
...
averaging over a large number of genes \citep{stoletzki_estimation_2011}. Across our
...
\citep{messer_frequent_2013} which might obscure the signal of positive selection. We
...
chromosomes at a telomere-to-telomere scale \citep{nurk_complete_2022,
...
\citep{cheng_haplotype-resolved_2022}, further work is likely needed before we can
...
\citep{merel_relaxed_2025,langmuller_genomic_2023,bourque_ten_2018}, following the
...
\citep{nunez_footprints_2025,li_comparative_2025,pool_population_2012,langley_genomic_2012}.
...
\texttt{inspector-correct.py} (Table~\ref{tab:sup_asm_stats}).
...
\citep{storer_dfam_2021}, extracting the annotated arthropod repeat sequences to generate
...
by extression levels (\texttt{--filter-min-expr "0.0"}).
...
We considered sequences as contaminant if they had high similarlity to bacteria, fungi, viridiplantae, archaea, nematoda, cestoda, and trematoda.
...
\citep{chan_trnascan-se_2021}. We specified the annotation of eukaryotic tRNA motiffs
...
We used whole-genome shotgun sequencing in order to assses the genetic diversity of
...
We reran \textsc{pixy} separately for each feature, specifiying the genomic coordinates of
the feature's annotation using the \texttt{--bed\_file} option.
We also calculated $\pi$ at zero-fold and four-fold degenerate sites.
...
After determining degeneracy we separatelty calculated $\pi$ for zero-and four-fold sites
using \textsc{pixy}, specifying the specific sites of interest using \texttt{--sites\_file}
and averaging within the span of each exon (using \texttt{--bed\_file}).
...
estmates of $\theta_\text{W}$ from \textsc{pixy}, and using the relationship
$\text{N}_\text{e} = \theta_\text{W}/4\mu$.
...
Using this relationship, we calculated \Ne{} along 10 kbp windows, obtaining a distribution
of effective size along the genome.
\vspace{0.2cm}
...
\textsc{DemInfHelper} program \citep{forest_popsize_2024}.
We ran \textsc{msmc2} with 25 iternations (\texttt{-i 25}), using the default
...
Since our genotypes were not phased, we ran \textsc{msmc2} for each individual separately
(including both our reference sample and our six central Oregon individuals).
...
Assembly and annotation on PRJNA1378260. Popgen Illumina data on PRJNA1434535.
