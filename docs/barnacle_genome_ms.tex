...
\citealp{messer_frequent_2013}) to correct for the biases introduced by low frequency
...
However, to ensure we recovered results for all seven allozymes, we removed individual
polymorphic sequences containing frameshift mutations instead of removing the whole
ortholog altogether.
...
\texttt{2.1} (\url{https://github.com/adelq/dnds}), calculates the number of synonymous
sites (S\textsubscript{C}) and synonymous substitutions (S\textsubscript{D}) at each
aligned codon.
...
(8 codons), calculating the average, Jukes-Cantor corrected \Ks{}, the average
$\pi_{4}$, and their ratio ($\pi_{4}/\text{K}_\text{S}$) for each window.
