...
% Trying to provide examples without being too listly. There's more gene to pull from,
% but I tried to focus on the most interesting ones based on the MKT results, i.e.,
% largest alphas, the example with excess polymorphism, and overlap with dn/ds.
...
Among other candidates showing large $\alpha$, we find a homolog to transmembrane channel-like 7
($\alpha = 0.985$), which has been linked to osmoregulation in crustaceans
...
While the functions of all of these candidate genes are unknown in \bgla{}, they highlight
the diverse targets of selection in this system and their characterization serves as
an avenue for future study in barnacles.
...
Among the seven allozyme loci, only \textit{Gpi} and \textit{Me.2} show a significant MK test
...
For both of these genes, both the NI and DoS statistics show that this deviation is due to an
excess of non-synonymous substitutions, indicating that these sequences evolved under a
regime of positive selection.
...
adaptively evolving protein-coding sequences in the \bgla{} genome.
\vspace{0.2cm}

The \textit{Gpi} result is particularly notable.
\cite{hedgecock_temporal_1994} had already identified \textit{Gpi} as one of several
\bgla{} allozyme loci showing significant microgeographic heterogeneity in allele
frequencies, but allozyme-era methods could not distinguish whether that heterogeneity
reflected protein-level selection, drift, or chaotic recruitment among sweepstakes-limited
larval cohorts \citep{hedgecock_does_1994}.
...
This pattern also contrasts with prior work at the \textit{Gpi} ortholog in
...
selective history.
\textit{Gpi} encodes a glycolytic enzyme whose activity sits at the branch point between
glycolysis and the pentose phosphate pathway; protein-coding variation at \textit{Gpi}
in other marine invertebrates has been linked to thermal tolerance and metabolic
flux under osmotic and thermal stress \citep{flight_allozyme_2012}, providing a
plausible functional axis along which directional selection could act in Pacific
intertidal populations.
\vspace{0.2cm}

The two malic-enzyme paralogs provide a second line of evidence that is worth noting
explicitly.
Only \textit{Me.2} shows a significant MK signal ($\alpha = 0.719$), while its paralog
\textit{Me.1} is indistinguishable from neutrality (Table~\ref{tab:sup_allozymes_mkt}).
Because the two copies of NADP-dependent malic enzyme share deep sequence homology but
differ in genomic context, this paralog-level asymmetry suggests that the positive
selection we detect is acting on \textit{Me.2} specifically rather than on the malic
enzyme gene family as a whole, and it cautions against treating classical ``allozyme
loci'' as monolithic units in genome-scale reanalyses.
\vspace{0.2cm}
...
% @Angel -- I've edited the text to be more cautious about the interpretation of the results
% I think the biggest thing we could do would be either a length match null comparison
% or a more formal HKA test to see if the excess of polymorphism is significant relative to the genome-wide background. 
% I think the windowed Pi_4/K_S results are suggestive but not diagnostic of balancing selection, and I don't want to overstate them without a more formal test. 
% That said, the fact that Mpi is the only one of the seven allozymes that shows an excess of polymorphism in the MK test 
% and also has a window with elevated Pi_4/K_S is suggestive that there might be something interesting going on at this locus, 
% even if we can't definitively say it's balancing selection at this point.

Turning to \textit{Mpi}, we observe the largest excess of polymorphism relative to
divergence among the seven allozymes (NI = 1.575), though the MK test at this locus
is not significant after correction ($p = 0.443$).
To further probe the landscape of variation at these loci, we examined the ratio of
synonymous diversity (in the form of $\pi$ at 4-fold degenerate sites; $\pi_{4}$) to
synonymous divergence ($\text{K}_\text{S}$) along windows spanning each coding sequence.
All seven allozyme loci contain at least one window whose $\pi_{4}/\text{K}_\text{S}$
value falls in the top 5\% of the genome-wide distribution
(Figure~\ref{fig:sup_gw_piks}), and these outlier windows tend to be contained within
the span of single exons (Figure~\ref{fig:sup_allozyme_piks}).
We note that with roughly 50 overlapping windows per gene, a single window exceeding
the 95th percentile of the genome-wide distribution is a weak test of locus-level
balancing selection and is largely expected by chance; we therefore interpret these
patterns as localized elevations in $\pi_{4}/\text{K}_\text{S}$ consistent with, but
not diagnostic of, balancing selection acting over short sequence scales.
\vspace{0.2cm}

This caveat aside, the \textit{Mpi} result does connect to earlier allozyme observations
in \bgla{}: \cite{hedgecock_temporal_1994} documented significant allele-frequency
heterogeneity at \textit{Mpi} across a 1-m intertidal transect at Bodega Harbor, a
pattern of microgeographic structure analogous to that subsequently documented at the
\textit{Mpi} ortholog in \textit{S. balanoides}
...
Taken together, the modest excess of non-synonymous polymorphism (NI $>$ 1) and the
localized elevation of $\pi_{4}/\text{K}_\text{S}$ at \textit{Mpi} are suggestive of
--- but by themselves do not establish --- a regime of balancing selection at this
locus in \bgla{}.
We further caution that intertidal microhabitat should be approximately uniform in our
central Oregon sampling, so we cannot directly test the habitat-based
balancing-selection hypothesis developed in \textit{S. balanoides}.
Future sampling of \bgla{} across its large geographic range, and across intertidal
microhabitats, together with formal tests such as the HKA test or matched-length
null distributions for the $\pi_{4}/\text{K}_\text{S}$ windows, will be needed to
determine whether the mechanism of balancing selection at \textit{Mpi} is shared
across these congeneric systems or reflects distinct evolutionary histories in
Atlantic and Pacific barnacles.
...
Transcript IDs and genomic coordinates of these orthologs are shown in
...
described above for all orthologs.
