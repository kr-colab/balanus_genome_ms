...
\commentauth{Angel}{Reduced the genome assembly and annotation sections, previously
two different sections with multiple paragraphs each, to just this one section. It skips
some of the details present in the methods, whole still describing the general details
...
a conserved synteny analysis using the software \textsc{Genespace}
\citep{lovell_genespace_2022}.
We compared our \bgla{} genome against the chromosome-level genome assemblies of the
...
\begin{figure}[ht]
    ...
    \caption{\textbf{Conserved synteny across barnacle genomes.}
        ...
        pollicipes} (bottom).
        Each set of colored lines shows syntenic orthologous genes across the three
        assemblies, colored according to their chromosome of origin.
...
In this reference individual, transcripts had a mean of 7.74 (median: 3, standard
deviation: 14.97) non-synonymous variants, indicating considerable coding variation
segregating within a single individual.
...
\commentauth{Angel}{Following one of Scott's comments, trying to minimize some of the more
discussion-y text here. Still kept some of the comparisons I thought were a bit more relevant.
We can put any of that text back if needed.}
...
patterns characterized by \cite{siepel_evolutionarily_2005} across vertebrate, insect, worm,
and yeast genomes.
While the raw composition of enriched conserved elements is different across all these
species, the consistent enrichment of CDSs is consistent with the expectation that
...
\begin{figure}[ht]
    ...
    \caption{\textbf{Highly conserved elements in the \balgla{} assembly.}
        ...
        are shown only for elements with proportions larger than 0.01. Sites that overlap
...
Oregon coast (see \nameref{sec:results_popgen}), comparing those against the orthologous
...
also shows strong signals of positive selection ($\alpha = 0.917$).
...
significant gene in the MK test overlaps with our candidates of selection in
...
four of the five loci showed significant allele-frequency heterogeneity at microgeographic
scales --- including heterogeneity at \textit{Mpi} across a single 1-m intertidal transect.
...
an excess of non-synonymous substitutions, suggesting that these sequences evolved under
...
rapidly evolving protein-coding sequences in the \bgla{} genome.
...
test (HKA; \citealp{hudson_test_1987}). For each locus we assembled a full-length alignment
between the sequences of \bgla{} and \textit{B. crenatus}, spanning the gene body, including
introns, plus 5\,kbp of flanking sequence on either side.
This alignment was then augmented with the \bgla{} ingroup polymorphism derived from our
population panel.
Testing each allozyme locus against the pooled remainder under the segregating-sites variant
of the test (Table~\ref{tab:sup_hka}), only \textit{Me.1} showed a nominally significant deviation
($p = 0.032$, uncorrected for multiple comparisons across seven loci), exhibiting a deficit
of polymorphism relative to its divergence --- the direction expected under a recent selective
sweep or strong purifying constraint at linked sites.
\vspace{0.2cm}
...
heterogeneously along the gene body at these loci, similar to the pattern observed
for \textit{Mpi} in \textit{S. balanoides} (see Figure~\ref{fig:sup_sliding_hka}
...
\begin{figure}[ht]
    ...
    \caption{\textbf{Sliding-window HKA diagnostic at \textit{Pgm} and \textit{Me.2}
        ...
        not be homologized; greys boxes) are omitted. The gene structure track below each
        panel shows exons (filled black blocks), untranslated regions (open outlined blocks),
        ...
        \commentauth{Angel}{Low priority, but could we get a scaled version of the figure
        in which the text is a bit larger?}
    }
...
$\pi$ $\approx$ $4.3\times$ lower than the genomic background and consistent with the
