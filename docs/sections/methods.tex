...
In addition to the seven allozyme loci, we repeated this analysis for all the 
...
\citep{kreitman_inferring_1991}, as implemented in the \textsc{sliding\_hka}
Python package \citep{kern_hka}. 
For each of the seven allozyme loci we constructed a per-locus full-length multiple
sequence alignment spanning the gene body plus 5\,kbp of flanking sequence on either side.
Coding regions were aligned codon-aware using the existing CDS alignments between
\bgla{} and \textit{B. crenatus} from the MK analysis; intronic and
intergenic flanks were aligned pairwise with \textsc{lastz} version 1.04.52
\citep{harris_lastz} under permissive sister-species parameters
(\texttt{--seed=match8} \texttt{--hspthresh=1500} \texttt{--gappedthresh=1000}
\texttt{--ydrop=9400} \texttt{--inner=2000}); the two tracks were stitched together
in genomic coordinates.
Per-sample ingroup haplotype sequences for the same windows were extracted
via \textsc{bcftools} \texttt{consensus} (\texttt{-H 1,2}) against the filtered
all-sites VCFs, then profile-aligned onto the stitched reference alignment
with \textsc{mafft} \texttt{--add} \texttt{--keeplength}
\texttt{--adjustdirectionaccurately} \citep{katoh_mafft_2013}.
For the classic HKA test each locus was tested against the pool of the remaining
six under the segregating-sites variant;
$T+1$ was estimated jointly across all seven loci.
CDS columns were treated codon-aware (Nei-Gojobori synonymous-site denominator and
path-based synonymous classification), while non-coding columns were treated per-site
...
\commentauth{Angel}{TODO for Angel: Add this to the README and script repo.}
