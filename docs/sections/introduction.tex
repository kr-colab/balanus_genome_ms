...
\commentauth{Angel}{Added the clonal interference stuff here to mention something else 
that we see from large N systems. Before we only had the hitchhiking example.}
...
\commentauth{Angel}{Same here, splitting this paragraph into two. Now one just introduces
glandula and the one below talks a bit more about previous research in the system.}
\vspace{0.2cm}
...
While applicable to long-term evolutionary comparisons (such as we do here), their distance
limits the applicability of these publicly available resources for studies in \bgla{}.
This drives the need for the generation of high-quality assemblies for both evolutionarily
closely related species, which could serve as outgroups for recent evolutionary
comparisons, as well as co-localized species which enable the study of ecological factors
maintining diversity in this system.
\commentauth{Angel}{Addressing Scott's comment. I moved some of this content around to
1. keep the focus on glandula before talking about other barnacles and 2. (hopefully) better
set up the need for crenatus (although I do not mention the species explicitly).}
\vspace{0.2cm}

\commentauth{Angel}{I moved some of the paragraphs above; however this paragraph now feels
a bit random here. Not sure if we should remove it since we do go back to the allozymes later.
I think we need some better transition out of glandula into other genetics work on barnacles
and then back to out ``summary'' paragraph at the end of the section.}
...
nunez_tides_2021}.
In the northern acorn barnacle (\textit{Semibalanus balanoides}), adaptive alleles in the
mannose-6-phosphate isomerase (\textit{Mpi}) gene are partitioned between low and high
intertidal zones less than five meters apart, with environmental heterogeneity maintaining a
balanced polymorphism \citep{schmidt_environmental_2000,nunez_footprints_2020}.
Further work has shown that different exons of \textit{Mpi} experience distinct selective
pressures across spatial and temporal scales, suggesting a more complex selective landscape
than the simple zonation model implies \citep{nunez_tides_2021}.
Despite the depth of this work, it has focused on a single protein-coding gene, and the
genome-wide landscape of genetic diversity and selection in \textit{S. balanoides} remains
unknown.
\vspace{0.2cm}

\commentauth{Angel}{Scott mentioned that this paragraph didn't flow as much with the
big Ne intro. I re-wrote some of the content to (hopefully) make the link a bit
stronger.}
...
acorn barnacle.
We use this assembly to perform whole-genome comparative analyses, revealing conservation
...
In addition to providing a high-quality genomic resource for a genomically understudied
group, our assembly lays the groundwork for future work in \bgla{} and other barnacles as
